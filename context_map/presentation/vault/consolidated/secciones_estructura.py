...
def _render_seccion_estructura(
    ...
    partes.extend([
        ...
        "        CLI[\"💻 CLI Entrypoint (cli.py)\"]",
        "        CMD[\"🛠️ Comandos (build, scan, sync, refresh, watch, doctor)\"]",
        "    end",
        "",
        "    subgraph DOM [\"🧠 Capa de Dominio (domain)\"]",
        "        SCAN[\"🔍 Escáner (security, cache, cyclomatic)\"]",
        "        SYNC[\"🔄 Sincronización & Watcher\"]",
        "        HEALTH[\"🏥 Health & Self-Healing Doctor\"]",
        "    end",
        "",
        "    subgraph CORE [\"⚙️ Capa Core (core)\"]",
        "        MODELS[\"📦 Dataclasses (Node, Edge, Event)\"]",
        "        TOKENS[\"📊 Tokenizador Oficial LLMs\"]",
        "        GEN[\"🎨 Generadores Narrativos con Alma\"]",
        "    end",
        "",
        "    subgraph INFRA [\"🔌 Infraestructura & Presentación\"]",
        "        MCP[\"🤖 Servidor MCP Nativo\"]",
        "        VAULT[\"🗺️ Bóveda Obsidian Jerárquica\"]",
        "        BRIEF[\"📑 Brief Ejecutivo CONTEXT.md\"]",
        "    end",
        "",
        "    CLI --> CMD",
        "    CMD --> SCAN",
